...
export class GeminiScriptProvider implements IScriptGenerator {
  ...
  constructor(config: { apiKey: string; model?: string }) {
    ...
    this.model = new ChatGoogleGenerativeAI({
      ...
      temperature: 0.7,
      maxRetries: 2,
      maxOutputTokens: 8192 // Limite razoável para scripts longos
    })
  }
  ...
  async generate(request: ScriptGenerationRequest): Promise<ScriptGenerationResponse> {
    console.log(`[Gemini Script] 🎬 Iniciando geração de roteiro via LangChain (${this.modelName})...`)

    // Configurar o modelo para output estruturado (Zod) com includeRaw para capturar token usage
    // Gemini supports structured output natively via LangChain
    ...
    if (request.images && request.images.length > 0) {
      console.log(`[Gemini Script] 👁️ Injetando ${request.images.length} imagens no contexto multimodal...`)
      ...
      request.images.forEach((img, idx) => {
        ...
        try {
          ...
          console.warn(`[Gemini Script] ⚠️ Falha ao converter imagem ${idx}. Erro: ${e}`)
        }

        if (!base64Data) {
          console.warn(`[Gemini Script] ⚠️ Imagem ${idx} ignorada: falha na extração de dados. Tipo: ${typeof img.data}`)
          return
        }
        ...
        if (!validMimeTypes.includes(mimeType)) {
          console.warn(`[Gemini Script] ⚠️ Imagem ${idx} ignorada: formato não suportado (${mimeType}).`)
          return
        }
    ...
    try {
      ...
      console.log('[Gemini Script] 📤 Enviando request multimodal para LangChain...')
      console.log('[Gemini Script] 🔍 Schema esperado: title, summary, scenes, backgroundMusic, backgroundMusicTracks')
      ...
      console.log(`[Gemini Script] 📥 Resposta recebida e validada em ${elapsed}s`)
      ...
      console.log(`[Gemini Script] 📊 Token Usage REAL: ${inputTokens} input + ${outputTokens} output = ${totalTokens} total`)

      // Validação rápida de integridade
      console.log('[Gemini Script] ✅ Roteiro gerado com sucesso!')
      console.log('[Gemini Script] Título:', content.title)
      console.log('[Gemini Script] Número de cenas:', content.scenes.length)
      console.log('[Gemini Script] Background Music:', content.backgroundMusic ? 'Sim (video todo)' : 'Não')
      console.log('[Gemini Script] Background Music Tracks:', content.backgroundMusicTracks?.length || 0, 'tracks')

      return this.parseResponse(content, request, { inputTokens, outputTokens, totalTokens })
    } catch (error) {
      console.error('[Gemini Script] ❌ Erro na geração estruturada:', error)
      console.error('[Gemini Script] 🔍 Error details:', JSON.stringify(error, null, 2))
      throw error
    }
  }
...
- DENSIDADE OBRIGATÓRIA: Com base na velocidade de fala (${targetWPM} WPM), cada cena DEVE conter entre ${minWords} e ${maxWordsHard} palavras. A conta é: ${targetWPM} WPM ÷ 60 × 5s = ${wordsPerScene} palavras ideais.
...
- PROIBIDO FRASES CURTAS: Cenas com menos de ${minWords} palavras geram "buracos" no áudio. Expanda com adjetivos, detalhes sensoriais ou contexto.
